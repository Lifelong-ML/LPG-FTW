...
class BatchREINFORCEEWC:
    ...
    def flat_vpg(self, observations, actions, advantages):
        ...
        theta = torch.cat((theta0, theta1)).data.numpy()

        tmp_grad = 2 * self.A.dot(theta) - 2 * self.b
        if self.T > 1:
            if self.scaled_lambda:
                vpg_grad[:self.d] += self.ewc_lambda / (self.T - 1) * np.squeeze(tmp_grad)
            else:
