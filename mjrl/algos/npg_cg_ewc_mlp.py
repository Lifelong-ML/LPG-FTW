...
from mjrl.algos.batch_reinforce_ewc_mlp import BatchREINFORCEEWC
...
class NPGEWC(BatchREINFORCEEWC):
    ...
    def grad_and_hess(self, observations, actions, advantages, theta):
        ...
        vpg_grad0 = torch.cat((vpg_grad[0], torch.unsqueeze(vpg_grad[1],1)), 1)
        vpg_grad0 = vpg_grad0.reshape((-1,1))
        vpg_grad1 = torch.cat((vpg_grad[2], torch.unsqueeze(vpg_grad[3],1)), 1)
        vpg_grad1 = vpg_grad1.reshape((-1,1))
        vpg_grad = torch.cat((vpg_grad0, vpg_grad1))
        ...
        kl_grad0 = torch.cat((kl_grad[0], torch.unsqueeze(kl_grad[1],1)), 1)
        kl_grad0 = kl_grad0.reshape((-1,1))
        kl_grad1 = torch.cat((kl_grad[2], torch.unsqueeze(kl_grad[3],1)), 1)
        kl_grad1 = kl_grad1.reshape((-1,1))
        kl_grad = torch.cat((kl_grad0, kl_grad1))

        hess_cols = []
        for i in range(kl_grad.numel()):
            tmp = torch.autograd.grad(kl_grad[i], theta, retain_graph=(i < (kl_grad.numel()-1)))
            tmp0 = torch.cat((tmp[0], torch.unsqueeze(tmp[1], 1)), 1)
            tmp0 = tmp0.reshape((-1,1))
            tmp1 = torch.cat((tmp[2], torch.unsqueeze(tmp[3], 1)), 1)
    ...
    def add_approximate_cost(self, N,
        ...
        theta0 = list(self.policy.model.fc0.parameters())
        theta1 = list(self.policy.model.fc1.parameters())
        theta = theta0 + theta1

        self.grad_and_hess(observations, actions, advantages, theta)

        theta0 = torch.cat((theta0[0], torch.unsqueeze(theta0[1], 1)), 1)
        theta0 = theta0.reshape((-1,1))
        theta1 = torch.cat((theta1[0], torch.unsqueeze(theta1[1], 1)), 1)
        theta1 = theta1.reshape((-1,1))
        theta = torch.cat((theta0, theta1))

        self.theta[self.task_id] = theta.data.numpy()

        self.A += self.hess[self.task_id]
        self.b += self.hess[self.task_id].dot(self.theta[self.task_id])
