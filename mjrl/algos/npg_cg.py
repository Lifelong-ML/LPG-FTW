...
class NPG(BatchREINFORCE):
    ...
    def grad_and_hess(self, observations, actions, advantages):
        ...
        if len(theta) == 3:
            ...
            vpg_grad = torch.cat((vpg_grad[0].view(-1,1 ), 
                                    vpg_grad[1].view(-1,1),
                                    vpg_grad[2].view(-1,1),
                                    vpg_grad[3].view(-1,1)))

            kl_grad = torch.cat((kl_grad[0].view(-1,1),
                                    kl_grad[1].view(-1,1),
                                    kl_grad[2].view(-1,1),
                                    kl_grad[3].view(-1,1)))
        ...
        lagrange = np.sqrt(vpg_grad.T.dot(FIM).dot(vpg_grad) / self.n_step_size * 2)
    ...
    def add_approximate_cost(self, N,
        ...
        if self.save_logs:
            self.logger.log_kv('time_sampling_hess', timer.time() - ts)
        ...
        self.grad_and_hess(observations, actions, advantages)
