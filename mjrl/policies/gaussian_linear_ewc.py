...
class LinearPolicyEWC:
    ...
    def set_task(self, task_id):
        ...
        first_time = self.task_id not in self.log_std
        if first_time:
        ...
        self.trainable_params = list(self.model.parameters()) + [self.log_std[self.task_id]]
        self.old_params = list(self.old_model.parameters()) + [self.old_log_std[self.task_id]]
        ...
        # -------------------------
        self.log_std_val = np.float64(self.log_std[self.task_id].data.numpy().ravel())
...
class LinearModel(nn.Module):
    ...
    def forward(self, x):
        # in_shift  = Variable(self.in_shift, requires_grad=False)
        # in_scale  = Variable(self.in_scale, requires_grad=False)
        # out_shift = Variable(self.out_shift, requires_grad=False)
        # out_scale = Variable(self.out_scale, requires_grad=False)
