...
job_name_ewc = 'results/ewc_std_shared_notscaled/halfcheetah_ewc_bodyparts_exp'
...
for i in range(num_seeds):
      ...
      for t in range(num_tasks):
            job_name_ewc_seed = job_name_ewc + '/seed_{}'.format(i)
            f = open(job_name_ewc_seed + '/iterations/task_{}/'.format(t) +  'policy_49.pickle', 'rb')
            policy_mtl = pickle.load(f)
            f.close()
            f = open(job_name_ewc_seed + '/iterations/task_{}/'.format(t) +  'baseline_49.pickle', 'rb')
