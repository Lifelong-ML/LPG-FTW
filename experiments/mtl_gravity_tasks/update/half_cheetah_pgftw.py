...
for i in range(num_seeds):
      ...
      for t in range(num_tasks):
            job_name_lpgftw_seed = job_name_lpgftw + '/seed_{}'.format(i)
            f = open(job_name_lpgftw_seed + '/iterations/task_{}/'.format(t) +  'policy_updated.pickle', 'rb')
            # f = open(job_name_lpgftw_seed + '/trained_mtl_policy.pickle', 'rb')
            policy_mtl = pickle.load(f)
            f.close()
            f = open(job_name_lpgftw_seed + '/iterations/task_{}/'.format(t) +  'baseline_49.pickle', 'rb')
            # f = open(job_name_lpgftw_seed + '/trained_mtl_baseline.pickle', 'rb')
            ...
            # policy_mtl.old_model.L.data = policy_mtl.model.L.data.clone()
      ...
      result_file = open(job_name_lpgftw_seed + '/forward_transfer_results.txt', 'w')
