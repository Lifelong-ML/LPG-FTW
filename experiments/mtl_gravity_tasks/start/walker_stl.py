...
job_name_stl = 'results/walker_stl_gravity_exp'
...
for i in range(num_seeds):
      ...
      for task_id in range(num_tasks):            
            # f = open(job_name_stl_seed + '/iterations/task_{}/'.format(t) +  'policy_updated.pickle', 'rb')
            f = open(job_name_stl_seed + '/iterations/task_{}/'.format(task_id) +  'policy_0.pickle', 'rb')
            # f = open(job_name_stl_seed + '/trained_mtl_policy.pickle', 'rb')
            policy_stl = pickle.load(f)
            f.close()
            f = open(job_name_stl_seed + '/iterations/task_{}/'.format(task_id) +  'baseline_0.pickle', 'rb')
            # f = open(job_name_stl_seed + '/trained_mtl_baseline.pickle', 'rb')
      ...
      # result_file = open(job_name_stl_seed + '/forward_transfer_results.txt', 'w')
