...
job_name_ewc = 'results/ewc_std_shared_notscaled/walker_ewc_gravity_exp'
...
for i in range(num_seeds):
      ...
      for t in range(num_tasks):
            job_name_ewc_seed = job_name_ewc + '/seed_{}'.format(i)
            f = open(job_name_ewc_seed + '/iterations/task_{}/'.format(t) +  'policy_199.pickle', 'rb')
            policy_mtl = pickle.load(f)
            f.close()
            f = open(job_name_ewc_seed + '/iterations/task_{}/'.format(t) +  'baseline_199.pickle', 'rb')
            baseline_mtl[t] = pickle.load(f)
            f.close()

            agent_mtl = NPGEWC(e, policy_mtl, baseline_mtl, ewc_lambda=1e-7, scaled_lambda=False, normalized_step_size=0.1, seed=SEED, save_logs=True)
