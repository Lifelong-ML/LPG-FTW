...
job_name_er = 'results/metaworldMT50_er_exp'
...
for i in range(num_seeds):
    ...
    for task_id in range(num_tasks):        
        ...
        f = open(iterdir +  'policy_0.pickle', 'rb')
        policy_mtl = pickle.load(f)
        f.close()
        f = open(iterdir +  'baseline_0.pickle', 'rb')
        baseline_mtl[task_id] = pickle.load(f)
        f.close()

        agent_mtl = NPGER(e, policy_mtl, baseline_mtl, capacity=500, normalized_step_size=0.01, seed=SEED, save_logs=True, gamma=0.995, gae_lambda=0.97)
    ...
    result_file = open(job_name_er_seed + '/start_results.txt', 'w')
